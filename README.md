...
- `heros_journey_game.html` — **雙語互動遊戲**（中／EN）。12 關依坎伯十二階段順序推進，含史詩敘事、抉擇、屬性養成（勇氣／智慧／心）、即時合成音效與環境氛圍。單檔內嵌所有圖片，離線可玩，**直接雙擊瀏覽器開啟**。
- `index.html` — 英雄旅程網頁（圓環主圖 + 12 階段插圖 + 中英說明）。**直接用瀏覽器開啟即可**。
- `images/` — 圖像資源
  - `00_map.png` — 圓環地圖主圖
  - `01…12_*.png` — 十二階段場景插圖（同一位綠斗篷主角、統一水彩風格）
- `gen_all.mjs` — 批次生成全部 12 張插圖的腳本
...
全部 12 張（可指定輸出資料夾／尺寸／品質；已存在的檔案會自動跳過）：
```powershell
